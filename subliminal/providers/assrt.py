...
language_converters.register('assrt = subliminal.converters.assrt:AssrtConverter')
...
class AssrtSubtitle(Subtitle):
    ...
    def _get_detail(self):
        ...
        for f in files:
            ...
            guess = guessit(f['f'], guessit_options)
            ...
            if self.language in langs:
                self._defail = f
                _, self.extension = os.path.splitext(f['f'])
                return f

        # second pass: keyword matching
        converter = language_converters['assrt']
        for f in files:
            langs = set([ Language(*converter.reverse(k)) for k in converter.codes if k in f['f'] ])
        ...
        # pick up first file if nothing matches
        f = files[0]
        _, self.extension = os.path.splitext(f['f'])
        return f
...
class AssrtProvider(Provider):
    ...
    def query(self, languages, video):
        ...
        if isinstance(video, Movie):
            ...
            if video.year:
                keywords.append(set(vidoe.year))
        ...
        r = self.session.get(server_url + '/sub/search', params=params, timeout=10)
        r.raise_for_status()
        ...
        for sub in result['sub']['subs']:
            ...
            for key in sub['lang']['langlist'].keys():
                ...
                try:
                    lang = converter.reverse(match.group('code'))
                    langs.add(lang)
                except:
                    pass

            subtitles.extend([AssrtSubtitle(Language(*l), sub['id'], sub['videoname'],
                                            self.session, self.token) for l in langs])
